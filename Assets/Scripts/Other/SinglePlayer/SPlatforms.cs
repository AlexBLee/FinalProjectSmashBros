...
public class SPlatforms : MonoBehaviour 
{
	...
	void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (tempObject.GetComponent<SPlayerManager>().dead && !done.Value && ready == false)
        {
	...
	void GoIdle()
	{
		tempObject.GetComponent<SPlayerManager>().SetDeath(false);
		ready = false;
